...
from Data_types.Sections import Sections
...
class CPSAT_variable_maker: # I am so sorry, there's so much nesting. Hopefully you learnt HTML?
                    ...
                    {other_class_sessions: model.new_int_var_from_domain(
                        ...
                        (
                            self.__time_to_index
                                (other_class_sessions.start_times)
                        ), 

                        f"start_{other_class_sessions.activity_name}_from_" +
    ...
    def __create_is_present_variables(self, courses: list[Course]) -> list[dict[str, dict[dict, Any]]]:
        ...
        return [semester_1, semester_2, both_semesters]


    def __assert_all_classes_accounted(self, start_time_variables, is_present_variables):
